...
Matesa.addCommand(
  { pattern: "getjids ?(.*)", fromMe: true, desc: "Jids adını al." },
  async (message, match) => {
    if (match === "")
      return await message.sendMessage(
        genButtons(
          ["GROUP", "PERSONAL"],
          "Grup veya Kişisel Jids Gösterir",
          "Choose"
        ),
        {},
...
Matesa.addCommand(
  { pattern: "engellemeyi kaldır ?(.*)", fromMe: true, desc: Lang.UNBLOCK_DESC },
  async (message, match) => {
    if (message.reply_message !== false) {
...
Matesa.addCommand(
  ...
  async (message, match) => {
    if (!message.reply_message)
      return message.sendMessage("*Bir mesaja cevap verin!*")
...
Matesa.addCommand(
  ...
  async (message, match) => {
    if (!message.reply_message || !message.reply_message.txt)
      return await message.sendMessage("*Bir kısa mesaja yanıt verin!*")

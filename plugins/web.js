...
`)
    ...
    if (jid == "list") {
      ...
      if (!all) return await message.sendMessage("*Görüntülenecek Program Yok*")
      let msg = ""
      all.forEach((jids) => {
        let {
          jid,
...
Hour      : ${hour}\nMinute    : ${minute}\nDate      : ${date}\nDayOfWeek : ${dayofweek}\nEnabled   : ${onoroff}\n\n`
    ...
    if (jid != undefined && !message.reply_message.txt && year == undefined) {
      ...
      if (info == false) return await message.sendMessage("*Bulunmadı*")
      let {
        jid: user,
        message: msgs,
        year,
...
Enabled: ${year}\n\n*Restart bot*`)
      ...
      return await message.sendMessage("*Söz dizimi hatası!*")
    } else {
      await setSchedule(
        jid,
        msg,
      ...
      await message.sendMessage(`${msg} için programlaşmış ${jid}\n\n*Restart bot*`)
    }
  }
)

...
Matesa.addCommand(
  ...
  async (message, match) => {
    ...
    if (!match) return await message.sendMessage("*Bana bir anahtar kelime verin.*")
    if (!vid)
    ...
    if (/^[0-9]+/.test(match)) {
      ...
      if (!url) return await message.sendMessage("*Başarısız*")
    ...
    if (!msg) return await message.sendMessage("*bulunamadı*")
...
Matesa.addCommand(
  {
    pattern: "score ?(.*)",
    fromMe: true,
    desc: "Canlı kriket skorunu gösterir ",
  },
  async (message, match) => {

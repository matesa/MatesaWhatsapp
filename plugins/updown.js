...
Matesa.addCommand(
  { pattern: "whois ?(.*)", fromMe: true, desc: "Grup veya kişi bilgilerini göster." },
  async (message, match) => {
...
Oluşturuldu : ${moment.unix(group.creation).format("MMMM Do YYYY, h:mm a")}
Açıklama    : ${group.desc}` +
        "```"
...
Matesa.addCommand(
  { pattern: "upload ?(.*)", fromMe: true, desc: "Bağlantıdan indir." },
  async (message, match) => {
...
Matesa.addCommand(
  ...
  async (message, match) => {
    ...
    const data = await IdentifySong(
      await message.reply_message.downloadAndSaveMediaMessage("bul")
    )
    if (!data) return

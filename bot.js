...
const { StringSession } = require("./Utilis/matesawhatsapp")
...
const MatesaWhatsappDB = config.DATABASE.define("MatesaWhatsapp", {
  info: {
...
async function MatesaWhatsapp(version) {
  await config.DATABASE.sync()
  let StrSes_Db = await MatesaWhatsappDB.findAll({
    where: {
      info: "StringSession",
    },
  })
...
${chalk.blue.italic.bgBlack("ℹ️ WhatsApp'a bağlanılıyor... Lütfen bekleyin.")}`)
  })
  conn.on("open", async () => {
    console.log(chalk.green.bold("✅ Giriş başarılı!"))
    console.log(chalk.blueBright.italic("⬇️ Pluginler yükleniyor..."))
    console.log(chalk.blueBright.italic("✅ Giriş bilgileri güncellendi!"))

    const authInfo = conn.base64EncodedAuthInfo()
    if (StrSes_Db.length < 1) {
      await MatesaWhatsappDB.create({
        info: "StringSession",
    ...
    plugins.map(async (plugin) => {
      try {
        ...
        console.log(
          `pluginler yüklenemedi : ${plugin.dataValues.name}`
        )
      }
    })
    console.log(chalk.blueBright.italic("⬇️  pluginler yükleniyor..."))
    ...
    console.log(chalk.green.bold("✅ Pluginler yüklendi!"))
  ...
  try {
    ...
    if (!nodb) {
      console.log(chalk.red.bold("Eski versiyon stringiniz yenileniyor..."))
      conn.loadAuthInfo(Session.deCrypt(config.SESSION))
      try {
        await conn.connect()
      } catch (e) {
